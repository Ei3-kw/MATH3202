...
# total milk m produced on day d
W = {(m,d): model.addVar() for m in M for d in D}

# total milk m sold on day d
X = {(m,d): model.addVar() for m in M for d in D}

# total whole milk produced on day d from farm f
Y = {(f,d): model.addVar() for f in F for d in D}

# total low fat milk produced on day d from farm f 
Z = {(f,d): model.addVar() for f in F for d in D}
...
for d in D:
    for m in M:
        if d > 0:
            model.addConstr(X[m,d] == W[m,d] - (S[m,d] - S[m,d-1]))
        else:
            model.addConstr(X[m,d] == W[m,d] - S[m,d])
...
for m in M:
    if d > 0:
        model.addConstr(S[m,d] == S[m,d-1] + W[m,d] - X[m,d])
    else:
        model.addConstr(S[m,d] == W[m,d]  - X[m,d])
