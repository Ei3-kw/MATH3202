...
MaxLow = 25     # max percentage of product that can be low fat (%)
MaxOrg = 15     # max percentage of product that can be organic (%)
...
m.setObjective(gp.quicksum(O_w * w[f] + O_l * x[f] +        # income from organic milk 
                           C_w * y[f] + C_l * z[f]          # income from normal milk 
...
# low fat milk is at most 25% of the total for each of organic and normal products
m.addConstr((MaxLow / 100) * gp.quicksum(w[f] + x[f] for f in F) >= gp.quicksum(x[f] for f in F))
